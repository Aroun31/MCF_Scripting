...
#define JIP(NAME,TARGET) class NAME { \
	...
	class CfgRemoteExec
	{
		class Functions
		{
			...
			F(DA3F_fnc_receveidItems, CLIENT)
			F(DA3F_fnc_receveidMoney, CLIENT)
			F(DA3F_fnc_getPriceBrourse, SERVER)
			F(DA3F_fnc_initPrice, SERVER)
			F(DA3F_fnc_updateIndiceBourse, SERVER)
			F(DA3F_fnc_loadSellMenu, CLIENT)
		};
